...
DEFAULT_CLIP_MULTIPLIER = 0.08
DEFAULT_KAPPA_INIT = 1.7
...
def run_mean_shift_full_data_init(
    data,
    h,
    ms_T,
):
    """
    Run ordinary non-private mean shift using the entire dataset as the
    initialization set. This MS baseline is independent of epsilon and does
    not use DAP or any private initialization.
    """
    ...
    return ms_est, ms_time


def run_dp_grams_with_dap_init(
    data,
    epsilon,
    delta,
    h,
    ...
    """
    Run DP-GRAMS with its private DAP initialization.

    Returns
    -------
    dp_est, dp_time, init_info, h_used
    """
    ...
    return dp_est, dp_time, init_info, h_used
...
def mse_vs_n_samples(n_samples_list, epsilon, delta, p, n_runs,
    ...
    for idx, n_samples in enumerate(n_samples_list):
        ...
        for run in range(n_runs):
            ...
            ms_est, ms_time = run_mean_shift_full_data_init(
                data=data,
                h=h,
                ms_T=ms_T,
            )

            dp_mse, dp_time = _run_dp_grams_mse_single(
                data=data,
...
def mse_vs_epsilon_for_n_samples(n_samples_list, epsilon_values,
    ...
    For MS, each run uses the whole dataset as the initial-mode set. This is a
    non-private, epsilon-independent baseline, so its MSE is repeated across
    epsilon values. DP-GRAMS is still recomputed for every epsilon and run.
    """
    mse_dict = {}
    ...
    for idx, n_samples in enumerate(n_samples_list):
        ...
        # Epsilon-independent non-private MS baseline for this n.
        # It is still evaluated over n_runs, but every MS run starts from
        # initial_modes=data.copy(), never from private DAP anchors.
        ms_mses_once, ms_times_once = [], []
        for run in range(n_runs):
            ms_est, ms_time = run_mean_shift_full_data_init(
                data=data,
                h=h,
            ...
            ms_mses_once.append(mode_matching_mse(true_modes, ms_est))
            ms_times_once.append(ms_time)

        ms_mean_fixed = float(np.mean(ms_mses_once))
        ms_se_fixed = float(np.std(ms_mses_once, ddof=1) / math.sqrt(n_runs))
        ms_time_mean_fixed = float(np.mean(ms_times_once))
        ms_time_se_fixed = float(np.std(ms_times_once, ddof=1) / math.sqrt(n_runs))
        ...
        for eps_idx, eps in enumerate(epsilon_values):
            ...
            ms_means.append(ms_mean_fixed)
            ms_ses.append(ms_se_fixed)
            ms_time_means.append(ms_time_mean_fixed)
            ms_time_ses.append(ms_time_se_fixed)
...
def mse_runtime_table(n_samples_list, epsilon_values,
                      ...
                      init_epsilon_frac=DEFAULT_INIT_EPSILON_FRAC):
    """
    Save MSE/runtime table over (n, epsilon).

    The MS baseline starts from the full dataset as initial modes and is
    epsilon-independent. DP-GRAMS is recomputed for each epsilon.
    """
    ensure_dir(results_dir)
    ...
    with open(csv_path, "w", newline="") as f:
        ...
        for idx, n_samples in enumerate(n_samples_list):
            ...
            ms_mse_mean = float(np.mean(ms_mses_once))
            ms_mse_se = float(np.std(ms_mses_once, ddof=1) / math.sqrt(n_runs))
            ms_time_mean = float(np.mean(ms_times_once))
            ms_time_se = float(np.std(ms_times_once, ddof=1) / math.sqrt(n_runs))
...
def plot_eta_results_grid(eta_results_by_n, n_list, out_path):
    ...
    fig.suptitle(r"MSE vs $\eta$ across n for 4-Modal Bivariate Gaussian Mixture", y=0.98)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    plt.savefig(out_path, dpi=120)
...
def main():
    ...
    for run in range(n_runs):
        ...
        dp_est, dp_time, init_info_run, h_used = run_dp_grams_with_dap_init(
            data=data,
            epsilon=epsilon,
            delta=delta,
            h=h,
    ...
    stats = (
        ...
        f"eta: {eta}\n"
        f"MS initialization: full dataset as initial_modes\n\n"
